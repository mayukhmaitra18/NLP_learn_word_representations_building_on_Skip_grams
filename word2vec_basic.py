...
def generate_batch(data, batch_size, num_skips, skip_window):
    ...
    """ 
      ...
      ===============================================================================
    """
    center, start = skip_window, 0
    # print('center',center)
    while start < batch_size:
        x, y = center - 1, center + 1
        for x in (center - skip_window, -1, -1):
            batch[start], labels[start] = data[center], [data[x]]
            start += 1
        for y in (0, batch_size):
            if y <= (center + skip_window):
                batch[start], labels[start] = data[center], [data[y]]
                start += 1
            else:
                break
        center += 1
